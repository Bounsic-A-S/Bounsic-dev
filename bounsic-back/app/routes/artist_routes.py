...
async def get_songs_by_artist(artist: str):
    res = await Artist_controller.get_songs_by_artist_controller(artist)
    if "error" in res:
...
async def get_artist_description(artist: str):
    res = await Artist_controller.get_artist_desc_controller(artist)
    if "error" in res:
...
async def get_artist_by_user_preferences(request: Request):
    ...
    res = await Artist_controller.get_artist_user_preferences_controller(email)
    return res